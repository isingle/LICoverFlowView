...
- (void)reloadData {
    ...
    self.contentOffset = CGPointMake(510.0, 0.0);//初始化居中
}
...
- (NSUInteger)nearByIndexOfScrollViewContentOffset:(CGPoint)contentOffset {
    ...
    NSLog(@"index==%d====%d",index,MIN(MAX(0, index), self.numberOfCoverViews-1));
...
- (CGPoint)nearByOffsetOfScrollViewContentOffset:(CGPoint)contentOffset {
    ...
    NSLog(@"scrollview.x====%f",index*(self.coverSize.width + self.coverSpace));
